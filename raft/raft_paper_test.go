...
func TestLeaderStartReplication2AB(t *testing.T) {
	...
	if g := r.RaftLog.unstableEntries(); !entsEqual(g, wents) {
...
func TestLeaderCommitEntry2AB(t *testing.T) {
	...
	if g := r.RaftLog.nextEnts(); !entsEqual(g, wents) {
...
func TestFollowerAppendEntries2AB(t *testing.T) {
	...
	for i, tt := range tests {
		...
		if g := r.RaftLog.allEntries(); !entsEqual(g, wents) {
			t.Errorf("#%d: ents = %+v, want %+v", i, g, wents)
		}
		...
		if g := r.RaftLog.unstableEntries(); !entsEqual(g, wunstable) {

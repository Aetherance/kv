...
func (ps *PeerStorage) InitialState() (*eraftpb.HardState, *eraftpb.ConfState, error) {
	...
	confState := util.ConfStateFromRegion(ps.region)
	return raftState.HardState, &confState, nil
}
...
func (ps *PeerStorage) Snapshot() (*eraftpb.Snapshot, error) {
	...
	if ps.snapTriedCnt >= 5 {
		err := fmt.Errorf("failed to get snapshot after %d times", ps.snapTriedCnt)
		ps.snapTriedCnt = 0
		return nil, err
	}
...
func (ps *PeerStorage) checkRange(low, high uint64) error {
	if low > high {
		...
		return fmt.Errorf("entries' high %d is out of bound, lastIndex %d",
...
func (ps *PeerStorage) Append(entries []*eraftpb.Entry, raftWB *engine_util.WriteBatch) error {
	...
	if lastEntry.Index >= first {
		for i := lastEntry.Index + 1; i <= ps.raftState.LastIndex; i++ {
...
func (ps *PeerStorage) ApplySnapshot(snapshot *eraftpb.Snapshot, kvWB *engine_util.WriteBatch, raftWB *engine_util.WriteBatch) (*ApplySnapResult, error) {
	...
	return &ApplySnapResult{PrevRegion: prevRegion, Region: snapData.Region}, nil
}

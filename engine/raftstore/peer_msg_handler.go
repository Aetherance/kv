...
func (d *peerMsgHandler) HandleRaftReady() {
	...
	if _, err := d.peerStorage.SaveReadyState(&rd); err != nil {
		log.Errorf("%s failed to save ready state %v", d.Tag, err)
		return
	}
		...
		if entry.Data != nil {
			...
			if req.AdminRequest != nil {
				...
				case raft_cmdpb.AdminCmdType_CompactLog:
					compactLog := req.AdminRequest.CompactLog
					if compactLog != nil && compactLog.CompactIndex > d.peerStorage.truncatedIndex() {
				...
				case raft_cmdpb.CmdType_Delete:
					kvWB.DeleteCF(req0.Delete.Cf, req0.Delete.Key)
					resp = &raft_cmdpb.Response{CmdType: cmdType}
	...
	if rd.SoftState != nil && d.peer.leaderID != 0 && d.peer.leaderID != d.LeaderId() {
		d.peer.leaderID = d.LeaderId()
